...
# KPIs
total_revenue = (df_filtered["Price"] * df_filtered["Sales_y"]).sum()
roi = ((total_revenue - df_filtered["M_Spend"].sum()) / df_filtered["M_Spend"].sum()) * 100
...
fig = px.bar(df_filtered.groupby("Category").sum(numeric_only=True).reset_index(),
             x="Category", y="Sales_y", color="Category",
